...
//			$page .= implode(' | ', $sort_options).'</p>';
	...
	elseif (isset($_GET['category'])) {
		...
		if (($category==strtolower($uncategorized) || $category==0) && $show_uncategorized) {
			...
			$page .= 'ddd<h'.$base_heading_level.'>'.ucwords($uncategorized).' ('.$count.') <small><a href="'.get_permalink( $post->ID ).'">&laquo;&nbsp;'.$main_page_back_text.'</a></small></h'.$base_heading_level.'>';
					...
					foreach ($cat->direct_decendents as $child_cat) {
						if ($category_array[$child_cat]->name) {
							...
							$scount = $wpdb->get_var("
								...
								AND $wp_dlm_db_taxonomies.taxonomy = 'category';
							");	
							if ($scount>0) $subcats[] = '<a href="'.wp_dlmp_append_url('category='.urlencode(strtolower($category_array[$child_cat]->id))).'">'.wptexturize($category_array[$child_cat]->name).'</a>';
						}					
					}
					if (sizeof($subcats)>0) :
						$page .= '<p class="subcats"><strong>'.$subcat_text.'</strong><br/> ';
						$page .= implode(' <br/> ', $subcats).'</p>';
					endif;
				}
