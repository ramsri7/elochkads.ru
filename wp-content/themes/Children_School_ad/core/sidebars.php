    ...
    if (!art_dynamic_sidebar($name))  {
        ...
        if ($sidebar) {
            ...
            for($i = 0; $i < count($blocks); $i++){
                ...
                switch($block) {
                    case 'search':
                        echo art_get_block( __('Поиск', THEME_NS), art_get_search());
                    break;
                    case 'archive': 
                        ob_start();
                        wp_get_archives('type=monthly&title_li=');// 2.6 not supported echo=0
                        echo art_get_block( __('Архивы', THEME_NS), '<ul>'.ob_get_clean().'</ul>');
                    break;
                    case 'categories':
                        echo art_get_block( __('Рубрики', THEME_NS), '<ul>'.wp_list_categories('show_count=1&title_li=&echo=0').'</ul>');
                    break;
