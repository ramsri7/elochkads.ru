...
if (post_password_required()) { 
	art_post_box('', '<p class="nocomments">' . __('Эта запись защищена паролем. Введите пароль, чтобы просмотреть комментарии.', THEME_NS) . '</p>');
	return;
}

if (have_comments()){
    ob_start();
    comments_number(__('Комментариев нет', THEME_NS), __('1 комментарий', THEME_NS), __('% комментариев', THEME_NS));
    art_page_navi('<span id="comments">' . ob_get_clean() . ' ' . sprintf(__('к записи &#8220;%s&#8221;', THEME_NS), the_title('', '', false)) . '</span>', true);
...
<?php if ( $user_ID ) : ?>
<p><?php printf(__('Вы вошли как <a href="%1$s">%2$s</a>.', THEME_NS), get_option('siteurl') . '/wp-admin/profile.php', $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('Выйти из этого аккаунта', THEME_NS); ?>"><?php _e('Выйти &raquo;', THEME_NS); ?></a></p>
<?php else : ?>
