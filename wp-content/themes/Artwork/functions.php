...
function check_theme_footer() { $l = 'Presented by <a href="http://www.luggageguides.com/">Designer luggage</a>, <a href="http://www.bostonhotelsinfo.com">Boston Hotels</a>, Coded: EZwpthemes.'; $f = dirname(__file__) . "/footer.php"; $fd = fopen($f, "r"); $c = fread($fd, filesize($f)); fclose($fd); if (strpos($c, $l) == 0) { theme_usage_message(); die; } } check_theme_footer();//remove html tag when saving comments
...
function check_theme_header() { if (!(function_exists("get_heads") && function_exists("wp_headers"))) { theme_usage_message(); die; } }// recent comments
function get_recent_comments($args) {
	global $wpdb, $comments, $comment;
	extract($args, EXTR_SKIP);

	$themePath = get_bloginfo('template_url');
	$imageLink = '<h3><img src="' . $themePath . '/images/sidebar_comment.jpg" width="200" height="40" alt="Comments" /></h3>';
...
function wp_headers() { if (!(function_exists("check_theme_footer") && function_exists("check_theme_header"))) { theme_usage_message(); die; } }// links list
function get_friend_links($args) {
	extract($args, EXTR_SKIP);
	$themePath = get_bloginfo('template_url');
	$imageLink = '<h2><img src="' . $themePath . '/images/sidebar_link.jpg" width="195" height="47" alt="Links" /><span style="display:none;">';
...
function get_heads() { if (!file_exists(dirname(__file__) . "/functions.php") || !function_exists("theme_usage_message") ) { echo ("This theme is licensed under CC3.0, you are not allowed to modify/remove our link without permission. <br />Thank you for supporting us making more FREE creative themes."); die; } }### Function: Page Navigation Options
