...
$themename = "ThatsMyBaby";
$shortname = "ThatsMyBaby";
$default_footer_content = "<a href='#'>Контакты</a> | <a href='#'>Правила использования</a> | <a href='#'>Торговые марки</a> | <a href='#'>Приватность</a><br /> &copy; 2011 ".get_bloginfo('name').". Все права защищены.";
...
function art_add_admin() {
    ...
    add_theme_page("Футер", "Футер", 'edit_themes', basename(__FILE__), 'art_admin');
...
<div class="postcontent">
      ...
      <div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s - %2$s'), get_comment_date(),  get_comment_time()) ?></a><?php edit_comment_link('('.__('Править', 'kubrick').')','  ','') ?></div>
...
if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."' class=\"prev\">&laquo; Previous</a>";
for ($i=1; $i <= $pages; $i++) {
if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )) {
echo ($paged == $i)? "<span class=\"current\">".$i."</span>":"<a href='".get_pagenum_link($i)."'>".$i."</a>";
}
}
if ($paged < $pages && $showitems < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\" class=\"next\">Next &raquo;</a>";
...
if ( '' == $text ) {
    ...
    $excerpt_word_count = 155; /*** MODIFY THIS. change the excerpt word count to any integer you like.***/
    $excerpt_length = apply_filters('excerpt_length', $excerpt_word_count);

    $excerpt_end = ' &#8230;<b><a href="'. get_permalink($post->ID) . '">' . '&raquo; Continue Reading: '. get_the_title() . '</a></b><br />';
